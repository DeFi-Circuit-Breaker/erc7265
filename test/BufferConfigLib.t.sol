...
contract BuffConfigLibTest is Test {
    function test_fuzzingConfigCreation(int256 maxDrawWad, uint256 mainWindow, uint256 elasticWindow) public {
        if (
            maxDrawWad >= int256(1e18) || maxDrawWad <= int256(-1e18) || mainWindow > type(uint32).max
                || elasticWindow > type(uint32).max
        ) {
            vm.expectRevert(BufferConfigLib.InvalidConfig.selector);
            BufferConfigLib.newConfig(maxDrawWad, mainWindow, elasticWindow);
        } else {
            BufferConfig config = BufferConfigLib.newConfig(maxDrawWad, mainWindow, elasticWindow);
            assertEq(
